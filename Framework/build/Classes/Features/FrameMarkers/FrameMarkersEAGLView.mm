...
- (id)initWithFrame:(CGRect)frame appSession:(SampleApplicationSession *) app
{
    ...
    if (self) {
        ...
        sampleAppRenderer = [[SampleAppRenderer alloc] initWithSampleAppRendererControl:self deviceMode:Vuforia::Device::MODE_AR stereo:false nearPlane:1 farPlane:5000.0];
...
- (void)renderFrameWithState:(const Vuforia::State &)state projectMatrix:(Vuforia::Matrix44F &)projectionMatrix
{
    ...
    for(int i = 0; i < state.getNumTrackableResults(); ++i) {
        ...
        if (!trackableResult->isOfType(Vuforia::MarkerResult::getClassType())) {
            continue;
        }
        const Vuforia::MarkerResult* markerResult = static_cast<
        ...
        SampleApplicationUtils::checkGlError("FrameMarkerss renderFrameVuforia");
    }
