...
- (void)viewWillAppear:(BOOL)animated
{
    ...
    [[NSNotificationCenter defaultCenter] addObserver:self selector:@selector(popPresentPage) name:@"popPresentPage" object:nil];
     [vapp startAR:Vuforia::CameraDevice::CAMERA_DIRECTION_BACK error:nil];
}
...
- (void)viewWillDisappear:(BOOL)animated
{
    ...
    cv.glResourceHandler = nil;
    self.arr = cv.navigateName;
...
- (void) showLoadingAnimation {
    ...
    if (orientation == UIInterfaceOrientationPortrait || orientation == UIInterfaceOrientationPortraitUpsideDown ) {
        indicatorBounds = CGRectMake(smallerBoundsSize / 2 - 30,
                                     largerBoundsSize / 2 - 90, 24, 24);
    }
    else {
        indicatorBounds = CGRectMake(largerBoundsSize / 2 - 30,
                                     smallerBoundsSize / 2 - 90, 24, 24);
    }
...
- (bool) doLoadTrackersData {
    ...
    NSString *searchFilename = @"Quizment.xml"; // name of the PDF you are searching for
    ...
    while (documentsSubpath = [direnum nextObject])
    {
        ...
        currentDataSet = [self loadObjectTrackerDataSet:[NSString stringWithFormat:@"%@/%@", documentsDirectory, documentsSubpath]];
    }
